...
BASE_DIR = r"C:\\Users\\aksha\\OneDrive\\Documents\\Desktop\\‌miniproject00"
...
def load_todays_sentiment(ticker, sentiment_file = os.path.join(BASE_DIR, 'mps', 'sentiment.csv')):
    try:
...
def fetch_stock_data(ticker, start_date, end_date, local_file_template = os.path.join(BASE_DIR, 'mps', 'stock_data_{ticker}.csv')):
...
def plot_stock_prices(ticker, df, predicted_prices, pred_dates):
    try:
        ...
        # Save plot
        plot_file = os.path.join(BASE_DIR, 'mps', f'stock_price_plot_{ticker}.png')
...
def predict_price():
    ...
    try:
        ...
        output_file = os.path.join(BASE_DIR, 'mps', 'stock_predictions.csv')
...
def serve_plot(filename):
    return send_file(os.path.join(BASE_DIR, 'mps', filename))
